...
def load_data():
    ...
    try:
        ...
        sheet_name = "Sheet1" if "Sheet1" in xls.sheet_names else xls.sheet_names[0]  # Use first sheet if "Sheet1" is not found
        ...
        df_clean = df.iloc[3:, [13, 17]].dropna()
        df_clean.columns = ["M/TSR Name", "0-7 Days Customers"]
        df_clean["0-7 Days Customers"] = pd.to_numeric(df_clean["0-7 Days Customers"], errors="coerce")
...
def populate_tree(df):
    ...
    for _, row in df.iterrows():
        tree.insert("", tk.END, values=(row["M/TSR Name"], row["0-7 Days Customers"]))
...
tree = ttk.Treeview(root, columns=("M/TSR Name", "0-7 Days Customers"), show="headings")
